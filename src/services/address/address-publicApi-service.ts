...
const addressHost = "https://online-gateway.ghn.vn/shiip/public-api/master-data"
...
class AddressPublicApiService implements IAddressPublicApiService {
  ...
  async getDistricts(provinceId?: number): Promise<ApiResponse<IDistrict[]>> {
    return (await fetch(`${addressHost}/district?province_id=${provinceId}`, requestInit)).json()
  }

  async getWards(districtId?: number): Promise<ApiResponse<IWard[]>> {
    return (await fetch(`${addressHost}/ward?district_id=${districtId}`, requestInit)).json()
  }
}
